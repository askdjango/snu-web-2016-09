...
print('개별 평균점수')
for name in result:
    print('%s님 국어: %d, 영어: %d, 수학: %d, 평균 : %f' %(name, int(result[name]['ko_score']), int(result[name]['en_score']), int(result[name]['math_score']), (int(result[name]['ko_score']) + int(result[name]['en_score']) + int(result[name]['math_score'])) / 3))
